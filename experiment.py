...
def exe_algs(part, t_max, precision, frac):
    ...
    with open("results/18-03/"+str(frac)+"/"+save_str+'.csv', 'a') as fo:
        cw = csv.writer(fo, delimiter=';')
        
        if sum(1 for row in csv.reader(open(
                "results/18-03/"+str(frac)+"/"+save_str+'.csv', 'r'))) == 0:
            print 'creating file', "results/18-03/"+str(frac)+"/"+save_str+'.csv'
            cw.writerow(['prob', 'n_v_sid', 'n_c_sid','n_c_solved_sid',
                     't_solved_sid', 'sur_prop_iters',  'result_sid',
                         'n_v_sim', 'n_c_sim', 'n_solved_sim', 't_solved_sim', 'result_sim'])
        else:
            print 'adding to', "results/18-03/"+str(frac)+"/"+save_str+'.csv'
